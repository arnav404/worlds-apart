...
public class FreeLookAddOn : MonoBehaviour
{
    ...
    public void OnLook(InputAction.CallbackContext context)
    {
        if(moveable) {
            ...
            //Ajust axis values using look speed and 0.00000001f so the look doesn't go faster if there is more FPS
            _freeLookComponent.m_XAxis.Value += lookMovement.x * LookSpeed * 0.01f;
            _freeLookComponent.m_YAxis.Value += lookMovement.y * LookSpeed * 0.01f;
        }
    }
}